...
const Opportunity = () => {
  ...
  const handleChangeSol = (e) => {
    e.preventDefault();
    if (e.target.value < 5111 && e.target.value >= 1) {
        ...
        {(currentDate === 'sol')
          ? (
            <div>
              ...
              <SolInput type="number" min={1} max={5111} value={sol} onChange={handleChangeSol} id="solinput" />
            </div>
          )
          : (
            <EarthDate
              ...
              minDate={new Date('2004-01-26')}
              maxDate={new Date('2018-06-11')}
